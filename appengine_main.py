...
class ShowLinks(webapp2.RequestHandler):
    def get(self, param):
        ...
        context = { "links": links, "is_admin": is_admin, "sign_out_link": sign_out_link, "fqdn": config.GOLINKS_FQDN, "hostname": config.GOLINKS_HOSTNAME }
...
class EditLink(webapp2.RequestHandler):
    ...
    def get(self, link):
        ...
        sign_out_link = users.create_logout_url('/')
        is_admin = users.is_current_user_admin()
        context = { "sign_out_link": sign_out_link, "is_admin": is_admin, "show_visibility": config.ENABLE_GOOGLE_GROUPS_INTEGRATION, 'hostname': config.GOLINKS_HOSTNAME }
        if link:
            link = link.rstrip("/")
            ...
            if l:
                if l.owner_id:
                    if l.owner_id != user.user_id() and not is_admin:
                        errorPage(self.response, 403, "Access denied")
                        return
                context.update({'url': l.url, 'viewcount': l.viewcount, 'public': l.public, 'visibility': l.visibility or '', 'can_delete': 1, 'owner': l.owner_name})
                            ...
                            for group in groups:
                                if group:
                                    try:
                                        ...
                                        if gsuite.directory_service.members().hasMember(groupKey=group, memberKey=user.email()).execute()['isMember']:
                                            no_access = False
                                            break
                                    except HttpError:
                                        pass
                            if no_access:
                                errorPage(self.response, 403, "You do not have access to the requested resource")
                                return
                l.viewcount += 1
                l.put()
